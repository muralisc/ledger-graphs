...
cat /dev/null > graph4b_daily_lastmonth_budget.tmp
...
(cat <<EOF) | gnuplot
  ...
  set output "$FOLDER/graph4b_daily_lastmonth_budget.png"
  set xdata time
  set timefmt "%Y-%m-%d"
  set format x "%d/%m/%Y"
  set format y "%g"
  ...
  set grid back ls 12
  set title "Daily LastMonth Expenses vs Budget as on ${ledger_run_date}"
  ...
  plot \
    ...
    "graph4b_daily_lastmonth_budget.tmp" using 1:2 with linespoints title "Budget" ls 1 linecolor rgb "blue", \
                     '' using 1:2:(sprintf("%'g", \$2)) with labels left font "Courier,12" rotate by 45 offset 1,1 textcolor "blue" notitle
EOF
popd
